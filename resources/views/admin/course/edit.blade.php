...
@section('content')
    <div class="container-fluid mt-4">
        ...
        <div class="card">
            <h3 class="text-center mt-3">Edit Course</h3>
            <form action="{{route('course.update',$course->id)}}" class="p-4" method="post" enctype="multipart/form-data">
                @csrf
                @method('put')
                ...
                <div class="form-group">
                    ...
                    <select name="category" class="form-control">
                        ...
                        @foreach ($category as $c)
                            <option value="{{$c->id}}" {{old('category',$course->category_id) == $c->id ? 'selected' : ''}}>{{$c->name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="language[]" class="form-control" id="select2" multiple>
                        ...
                        @foreach ($language as $l)
                            <option value="{{$l->id}}"
                                @foreach ($course->language as $cl)
                                    {{collect(old('language',$cl->id))->contains($l->id) ? 'selected' : ''}}
                                @endforeach
                            >{{$l->name}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <div class="ms-3 mt-2">
                        <input type="radio" name="type" class="form-check-control" value="free" {{old('type',$course->type)=='free' ? 'checked' : ''}}> Free
                        <input type="radio" name="type" class="form-check-control" value="paid" {{old('type',$course->type)=='paid' ? 'checked' : ''}}> Paid
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="preview-img mt-3">
                        <img src="{{asset('courseImage/'.$course->image)}}" alt="" width="100px" class="img-thumbnail">
                    </div>
                </div>
