@extends('layout.app')
@section('content')
<section class="main-course my-5">
    <div class="container">
        <div class="mb-3">
            <button class="btn btn-danger free" data-course-type="free">Free Course</button>
            <button class="btn btn-danger paid" data-course-type="paid">Paid Course</button>
        </div>
        <div class="card p-4" style="background-color: #f1f2f6;">
            <div class="row">
                <div class="col-md-8 col-sm-12 col-12">
                    {{-- Filter Start --}}
                        <form action="">
                            <select name="" id="" class="btn btn-default me-2 category_search">
                                <option value="">Filter By Category</option>
                                @foreach ($category as $c)
                                <option value="{{$c->slug}}">{{$c->name}}</option>
                                @endforeach
                            </select>

                            <select name="" id="" class="btn btn-default me-2 mt-2 language_search">
                                <option value="">Filter By Language</option>
                                @foreach ($language as $l)
                                <option value="{{$l->slug}}">{{$l->name}}</option>
                                @endforeach
                            </select>

                            <input type="text" name="" id="" class="filter-text me-2" placeholder="Enter Course Title"/>

                            <input type="submit" value="Filter" class="btn btn-danger btn-sm filter_btn">
                        </form>
                    {{-- Filter End --}}
                        <div class="courseList"></div>
                </div>
                {{-- List Start --}}
                <div class="col-md-4 col-sm-12 col-12 mt-3">
                    <div class="card " style="background-color: #f1f2f6;">
                        <div class="card-body">
                            <h5 class="text-center text-muted">Categories</h5>
                            @foreach ($category as $c)
                                <button class="mt-1 badge badge-danger category border-0" data-category-slug="{{$c->slug}}">{{$c->name}}</button>
                            @endforeach
                        </div>
                    </div>

                    <div class="card mt-3" style="background-color: #f1f2f6;">
                        <div class="card-body">
                            <h5 class="text-center text-muted">Languages</h5>
                            @foreach ($language as $l)
                                <button class="mt-1 badge badge-danger language border-0" data-language-slug="{{$l->slug}}">{{$l->name}}</button>
                            @endforeach
                        </div>
                    </div>
                </div>
                {{-- List End --}}
            </div>
        </div>
    </div>
</section>
@endsection
@section('script')
    <script>
        $(function($){
            courseSearch();

            function courseSearch(){
                var category = $('.category_search').val();
                var language = $('.language_search').val();
                var text = $('.filter-text').val();

                $.ajax({
                    url: `/course-list?category=${category}&language=${language}&text=${text}`,
                    type:'GET',
                    success:function(res){
                        console.log(res);
                        $('.courseList').html(res);
                    }
                });

                $('.filter_btn').on('click',function(e){
                    e.preventDefault();
                    courseSearch();
                });

            }

             $('.category').on('click',function(){
                var category_slug = $(this).data('category-slug');
                console.log(category_slug);
                $.ajax({
                    url: `/course-list?category=${category_slug}`,
                    type:'GET',
                    success:function(res){
                        $('.courseList').html(res);
                    }
                });
            });

            $('.language').on('click',function(){
                var language_slug = $(this).data('language-slug');
                console.log(language_slug);
                $.ajax({
                    url: `/course-list?language=${language_slug}`,
                    type:'GET',
                    success:function(res){
                        $('.courseList').html(res);
                    }
                });
            });

            $('.free').on('click',function(){
                var type = $(this).data('course-type');
                console.log(type);
                $.ajax({
                    url: `/course-list?type=${type}`,
                    type:'GET',
                    success: function(res){
                        console.log(res);
                        $('.courseList').html(res);
                    }
                })
            });

            $('.paid').on('click',function(){
                var type = $(this).data('course-type');
                console.log(type);
                $.ajax({
                    url: `/course-list?type=${type}`,
                    type:'GET',
                    success:function(res){
                        $('.courseList').html(res);
                    }
                });
            });

            @if(session('error')){
                Toast.fire({
                icon: 'error',
                title: "{{session('error')}}"
                })
            }
            @endif
        });
    </script>
@endsection
